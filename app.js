...
app.get("/posts/:postid", function(req,res){
    let req_title = _.kebabCase(req.params.postid);
    let pageFound = false;
    posts.forEach(function(element){
        let arrayTitle = _.kebabCase(String(element.postTitle));
        if(req_title === arrayTitle){
          pageFound = true;
          res.render('post', {pageTitle: element.postTitle, pageBody: element.postContent});
        }      
    })
    if(!pageFound){
      res.render('404');
